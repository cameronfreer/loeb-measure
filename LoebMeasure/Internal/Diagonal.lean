...
D0.1 spike (issue #1, ADR-0001): the exact hypothesis behind Loeb-measure
diagonalization, and the content-free diagonal lemma itself.
...
Nonempty fibers are assumed only to produce values at irrelevant indices; this is the
representative-selection role recorded in the architecture's assumption boundaries.
Carrier injectivity/extensionality (M2) needs only nonempty fibers and **no**
incompleteness hypothesis; the two assumptions remain deliberately separate.

Declaration names are provisional until ADR-0001 is accepted.
-/
